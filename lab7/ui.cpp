...
{
	const std::vector<Organic_fragment>& vector_to_print = this->controller.Controller::get_admin_data();

	for (auto fragment : vector_to_print)//using range-based for loop
	{
		std::cout << fragment << "\n";
	}
}
...
{
	Organic_fragment& element_to_show = this->controller.Controller::next();
	std::cout << element_to_show << "\n";
}
...
void Console::ui_list_given_size(char* split_command)
{
	...
	std::vector<Organic_fragment> filtered_vector = this->controller.Controller::get_filtered_list(size, quantity_of_micro_fragments);

	for (auto fragment : filtered_vector)
	{
		std::cout << fragment << "\n";
	}
}	
...
{
	const std::vector<Organic_fragment>& vector_to_print = this->controller.Controller::get_user_data();

	for (auto fragment : vector_to_print)
	{
		std::cout << fragment << "\n";
	}
}