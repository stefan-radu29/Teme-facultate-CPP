...
*/
{
	...
	if (!in_file.is_open())
	{
		throw Repository_exception("file cannot be opened!\n");
	}
...
*/
{
	...
	for (auto fragment : this->admin_data)
	{
		if (fragment.Organic_fragment::get_identifier().compare(new_organic_fragment.Organic_fragment::get_identifier()) == 0)
			throw Repository_exception("element with same identifier already exists!");
	}
...
*/
{
	...
	throw Repository_exception("no organic fragment with such identifier exists!");
}
